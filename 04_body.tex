...
Já o processo inverso, do plano $uv$ para o espaço cartesiano pode ser calculado através do mapeamento direto das coordenadas e posicionando o centro de cada face na posição 1, perpendicular ao eixo correspondente. Isto é, a face 0 fica no ponto x=-1, a face 2 no ponto z=1, etc. conforme 
...
O codificador H.265 permite que o vídeo seja segmentado em ladrilhos internamente, restringindo o compensador de movimento para dentro de um ladrilho~\cite{ITU-T2018}. Como os ladrilhos não possuem dependência entre sí, esta técnica permite a decodificação em paralelo de todos os ladrilhos com uma única instância do decodificador. Os trabalhos apresentados em~\cite{Concolato2017, LeFeuvre2016} descrevem um sistema que se aproveita desta capacidade para segmentar a projeção em ladrilhos e utilizar o protocolo DASH SRD para a descrição da relação espacial entre os mesmos. Este recurso foi implementado no tocador do software GPAC\footnote{https://gpac.io/}, porém as modificações no decodificador não fazem parte do padrão H.265 nem do DASH e o uso em sistemas Android (o sistema operacional mais comum dos HMD) não funciona muito bem. Por isso a maioria dos pesquisadores que adotam a abordagem do streaming de ladrilhos adaptado ao viewport realiza o ladrilhamento antes da codificação.



\subsection{O Viewport (projeção gnomônica)}

A extração do viewport é feita através da projeção Gnomônica, também conhecida como projeção retilinear. Nesta projeção, os pontos da esfera são projetados a partir do centro da esfera a um plano tangente a esfera\footnote{https://www.wolframalpha.com/input?i=rectilinear+projection}.  Como o viewport é definido como a porção da esfera que é visível pelo usuário e a visão humano  é limitada ao campo de visão (do inglês FOV - \textit{ Field of View}), os HMD exibem ao usuário a projeção com um FOV de aproximadamente $110^\circ\times 90^\circ$. O plano que será projetado é tangente a esfera no centro do FOV e se move junto com a cabeça do usuário. 


Como o viewport é definido como a porção da esfera que é visível pelo usuário e a visão humano  é limitada ao campo de visão (do inglês FOV - \textit{ Field of View}), os HMD extraem o viewport com FOV de aproximadamente $110^\circ\times 90^\circ$. 

Para isto os ponto da esfera que pertencem ao viewport são projetados a um plano tangente a esfera no ponto $ P(0, 0, 1) $ utilizando a projeção Gnomônica ou Retilínea\footnote{https://www.wolframalpha.com/input?i=rectilinear+projection}, como pode ser visto na figura~\ref{fig:projecao_viewport}. 
