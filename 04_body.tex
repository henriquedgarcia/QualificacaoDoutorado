...
O codificador H.265 permite que o vídeo seja segmentado em ladrilhos internamente, restringindo o compensador de movimento e alocando os slices para dentro de um ladrilho~\cite{ITU-T2018}. Como os ladrilhos não possuem dependencia entre sí, esta técnica permite a decodificação em paralelo de todos os ladrilhos com uma única instância do decodificador. Os trabalhos apresentados em~\cite{Concolato2017, LeFeuvre2016} descrevem um sistema que se aproveita desta capacidade para segmentar a projeção em ladrilhos e utilizar o protocolo DASH SRD para a descrição da relação espacial entre os mesmos. Este recurso foi implementado no tocador do software GPAC\footnote{https://gpac.io/}, porém as modificações no decodificador não fazem parte do padrão e o suporte para sistemas Android (o sistema operacional mais comum dos HMD) não funciona muito bem. Por isso a maioria dos pesquisadores que adotam a abordagem do streaming de ladrilhos adaptado ao viewport realiza o ladrilhamento antes da codificação~.
