...
Atualmente, o streaming de vídeos esféricos depende principalmente da arquitetura de streaming de vídeo 2D tradicional. Assim, toda a esfera de vídeo precisa primeiro ser projetada em um plano usando alguma técnica de projeção (por exemplo, equirretangular ou cubemap) antes de ser comprimida com um codificador de vídeo 2D padrão, como H.264 ou VP9. Desta forma, para que o vídeo no HMD exiba uma boa qualidade, a projeção do vídeo deve possuir uma resolução de pelo menos 4K (2160 linhas), gerando alta taxa de transmissão, exigindo grande capacidade de processamento e armazenamento~\cite{Afzal2017}. Codificadores tradicionais como H.264 geralmente não são usados para resoluções maiores que 4K, sendo necessário também novas tecnologias como o codificador H.265 para a compressão de vídeos em resoluções superiores. 

Aplicativos como YouTube e Facebook geralmente usam as projeções equirretangular (do inglês ERP - Equirectangular Projection) ou projeção CubeMap (do inglês CMP - CubeMap Projection), codificados com H.264 ou VP9 e empregam o protocolo aberto da MPEG Dynamic Adaptive Streaming over HTTP (DASH). No DASH o servidor disponibiliza o vídeo em  segmentos de duração fixa, geralmente de 4 a 8 segundos de vídeo, codificados em multiplas qualidades/taxas de bits (quanto maior a qualidade, maior a taxa de bits do vídeo). No lado do cliente, o aplicativo utiliza um algoritmo de Adaptação da Taxa de Bits (do inglês ABR - Adaptive Bitrate) para decidir qual a melhor qualidade considerando os recursos do dispositivo, como largura de banda disponível, resolução da tela, capacidade de processamento, decodificador disponível, bateria, etc. No entanto, ao contrário dos vídeos 2D, um usuário só pode visualizar apenas uma fração de todo o vídeo esférico devido à limitação do campo de visão humano (FOV). Conforme mostrado na Figura~\ref{fig:viewport}(a), a região do vídeo correspondente ao que é exibido no HMD é chamada Viewport. Dependendo do fabricante do HMD o FOV pode variar, mas geralmentee geralmente possui $110^{\circ}$ de abertura horizontal e $\times90^{\circ}$ de abertura vertical, o que corresponde a apenas 16,6\% da esfera~\cite{Afzal2017}. Assim, diversos recursos do sistema serão desperdiçados se todo o vídeo esférico for codificado e transmitido.
...
Afim de economizar recursos, vários trabalhos~\cite{Zare2016, Graf2017, Hosseini2017, Xiao2018} sugerem uma abordagem de streaming em que a projeção é segmenta espacialmente em vários vídeos independentes chamados ladrilhos, de modo que o cliente possa solicitar somente os ladrilhos em uma região de interesse (do inglês ROI - Region Of Interest) que serão vistos pelo usuário nos próximos segundos. A figura~\ref{fig:viewport}(b) apresenta esta abordagem utilizando um padrão de ladrilhamento $4 \times 4$ com um ROI e uma janela de visualização correspondente. Desta forma, somente os vídeos tocados pelo viewport serão requisitados ou poderão ser requisitados com alta qualidade, enquanto os ladrilhos não vistos serão transmitidos em baixa qualidade ou mesmo não serão transmitidos. Ainda podemos observar que quanto maior a segmentação (e menor os ladrilhos), menos pixels ficarão fora do viewport, reduzindo ainda mais o desperdício. 

Porém, a partir desta abordagem surgem vários novos desafios. Como os chunks do DASH possuem duração fixa é necessário prever o comportamento do usuário durante toda essa duração. Uma predição errada solicitará os chunks errados, desperdiçando largura de banda e em alguns casos o vídeo poderá travar até que o chunk correto chegue. Além disso, a segmentação da projeção em ladrilhos independentes exigirá do cliente diversas instâncias do decodificador trabalhando em paralelo. Desta forma, quanto maior o ladrilhamento, mais decodificadores serão necessários instanciar. O ladrilhamento também impacta na codificação, pois quanto menor o ladrilho, menor o espaço de busca do compensador de movimento dos codificadores, reduzindo a eficiência da compressão e, por fim, aumentando a taxa de bits.
